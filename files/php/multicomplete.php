<?php
	...
	if(!empty($_POST) && isset($_POST['data']) && !empty($_POST['data']) && strlen($_POST['data'])>100 ){
		...
		preg_match_all('/<span id="cphMainBodyContainer_ctl.*?_lblQuestionNumber" class="LabelText">Question: 1 of (.*?)<\/span>/si', $data, $qcount);
	...
	preg_match_all('/<table id="cphMainBodyContainer_ct.*?"(.*?)<\/table>/si', $data, $matches);
	...
	foreach($matches[1] as $k=>$v){
		...
		preg_match_all('/<span id="cphMainBodyContainer_ct.*?_lblQuestionText" class="LabelText"><pre>(.*?)<\/pre><\/span>/si', $v, $matches2);
		...
		preg_match_all('/<label for="cphMainBodyContainer_ctl.._ctl2._."><pre>(.*?)<\/pre><\/label>/si', $matches[1][$k], $matches3);
