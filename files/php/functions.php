...
function parseHTML2DB($data,$id){
	...
	*/
	...
	preg_match_all('/<table id="ctl00_cphMainBodyContainer_ctl0._Tab"(.*?)<\/table>/si', $data, $matches);
	...
	foreach($matches[1] as $k=>$v){
		...
		preg_match_all('/<span id="ctl00_cphMainBodyContainer_ctl0._lblQuestionText" class="LabelText"><pre>(.*?)<\/pre><\/span>/si', $v, $matches2);
		...
		// Get the answers
		preg_match_all('/<span disabled="disabled" class="LabelText">(.*?)<\/span>/si', $matches[1][$k], $matches3);
